...
bool Raids::loadFromXml()
{
	...
	for (auto raidNode : doc.child("raids").children()) {
		...
		attr = raidNode.attribute("delay");
		if (attr) {
			interval = 
				
				
				<uint32_t>(attr);
			if (interval < RAID_MINTICKS) {
				interval = RAID_MINTICKS;
			}
		} else {
			std::cout << "[Error - Raids::loadFromXml] delay tag missing or zero (would divide by 0) for raid: "
			          << name << std::endl;
			continue;
		}

		if ((attr = raidNode.attribute("margin"))) {
			margin = fs::xml_parse<uint32_t>(attr.value()) * 60 * 1000;
		} else {
...
bool RaidEvent::configureRaidEvent(const pugi::xml_node& eventNode)
{
	...
	if (delayAttribute) {
		uint32_t delay = fs::xml_parse<uint32_t>(delayAttribute);
		if (delay < RAID_MINTICKS) {
			delay = RAID_MINTICKS;
		}
		this->delay = delay;
	}
	else {
		std::cout << "[Error - Raids::parseRaidNode] delay tag missing for event node: " << raceName << std::endl;
		return false;
	}
	return true;
}
...
bool SingleSpawnEvent::configureRaidEvent(const pugi::xml_node& eventNode)
{
	...
	if ((attr = eventNode.attribute("x"))) {
		position.x = fs::xml_parse<uint16_t>(attr.value());
	} else {
	...
	if ((attr = eventNode.attribute("y"))) {
		position.y = fs::xml_parse<uint16_t>(attr.value());
	} else {
	...
	if ((attr = eventNode.attribute("z"))) {
		position.z = fs::xml_parse<uint16_t>(attr.value());
	} else {
...
bool AreaSpawnEvent::configureRaidEvent(const pugi::xml_node& eventNode)
{
	...
	if ((attr = eventNode.attribute("radius"))) {
		...
		if ((attr = eventNode.attribute("centerx"))) {
			centerPos.x = fs::xml_parse<uint16_t>(attr.value());
		} else {
		...
		if ((attr = eventNode.attribute("centerz"))) {
			centerPos.z = fs::xml_parse<uint16_t>(attr.value());
		} else {
		...
		if ((attr = eventNode.attribute("fromx"))) {
			fromPos.x = fs::xml_parse<uint16_t>(attr.value());
		} else {
		...
		if ((attr = eventNode.attribute("fromy"))) {
			fromPos.y = fs::xml_parse<uint16_t>(attr.value());
		} else {
		...
		if ((attr = eventNode.attribute("fromz"))) {
			fromPos.z = static_cast<uint8_t>(fs::xml_parse<int32_t>(attr.value()));
		} else {
		...
		if ((attr = eventNode.attribute("tox"))) {
			toPos.x = fs::xml_parse<uint16_t>(attr.value());
		} else {
		...
		if ((attr = eventNode.attribute("toy"))) {
			toPos.y = fs::xml_parse<uint16_t>(attr.value());
		} else {
		...
		if ((attr = eventNode.attribute("toz"))) {
			toPos.z = static_cast<uint8_t>(fs::xml_parse<int32_t>(attr.value()));
		} else {
	...
	for (auto& monsterNode : eventNode.children()) {
		...
		if ((attr = monsterNode.attribute("minamount"))) {
			minAmount = fs::xml_parse<uint32_t>(attr.value());
		} else {
			minAmount = 0;
		}
		...
		if ((attr = monsterNode.attribute("maxamount"))) {
			maxAmount = fs::xml_parse<uint32_t>(attr.value());
		} else {
			maxAmount = 0;
		}
